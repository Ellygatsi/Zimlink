...
function ProviderLogo({ logo, name, initials, initialsColor }) {
  ...
  if (failed || !logo) {
    return (
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-neutral-200 dark:bg-neutral-800 md:h-14 md:w-14">
        <span className={`text-sm font-medium ${initialsColor} md:text-base`}>{initials}</span>
      </div>
    );
  }

  return (
    <div className="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-white p-2 dark:bg-neutral-950 md:h-14 md:w-14">
      <img
        src={logo}
        alt={`${name} logo`}
