...
# Mirrors the CORSMiddleware allow_origin_regex above. Unhandled exceptions
# are caught by Starlette's error-handling layer, which sits OUTSIDE
# CORSMiddleware — so CORS headers are never attached automatically to
# these responses. Without doing it manually here, every unhandled crash
# looks like a "CORS policy" error in the browser instead of showing the
# real error, which is exactly what was happening with /reloadly/topup.
...
async def catch_all_exception_handler(request: Request, exc: Exception):
    """
    Safety net for any error that isn't already an HTTPException.

    Without this, an unhandled crash (e.g. inside a Reloadly call) skips
    the CORS middleware entirely, and the browser reports a misleading
    "CORS policy" error instead of the real problem. This ensures every
    response — including crashes — carries proper CORS headers and a
    readable message.
    """
